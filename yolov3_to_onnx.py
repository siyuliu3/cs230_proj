...
#         print()
#     if 0: #checksum_reference is not None:
...
if COCO:
    layer_configs = parser.parse_cfg_file("./config/yolov3.cfg")
else:    
    layer_configs = parser.parse_cfg_file("./config/yolov3-voc.cfg")
...
output_tensor_dims['082_convolutional'] = [int(OUT), int(SIZE/32), int(SIZE/32)]
output_tensor_dims['094_convolutional'] = [int(OUT), int(SIZE/16), int(SIZE/16)]
output_tensor_dims['106_convolutional'] = [int(OUT), int(SIZE/8), int(SIZE/8)]
...
if COCO:
    weight_file_path = "./config/yolov3.weights"
else:
    weight_file_path = "./config/yolov3-voc_last.weights"   
 # = download_file(
 #    local_path= 'yolov3.weights',
 #    link='https://pjreddie.com/media/files/yolov3.weights',
    # checksum_reference='c84e5b99d0e52cd466ae710cadf6d84c')
...
yolov3_model_def = builder.build_onnx_graph(
    ...
    weights_file_path=weight_file_path,
    verbose=True)
...
if COCO:
    # output_file_path = './engine/yolov3.onnx'
    output_file_path = os.path.join('./engine/onnx/', 'yolov3-' + str(SIZE) + '.onnx')
else:    
    # output_file_path = './engine/yolov3-voc.onnx'
    output_file_path = os.path.join('./engine/onnx/', 'yolov3-voc-' + str(SIZE) + '.onnx')
